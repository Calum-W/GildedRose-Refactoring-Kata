...
describe "Depreciating item" do
  describe "#update_quality" do
    ...
    it "only increases the value to 50 when sell_in is between 1-5 (min)" do
      bsp = DepreciatingItem.new(name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 1, quality: 48)
      expect { bsp.update_quality() }.to change { bsp.quality }.by(2)
    end

    it "only increases the value to 50 when sell_in is between 1-5 (max)" do
      bsp = DepreciatingItem.new(name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 5, quality: 48)
      expect { bsp.update_quality() }.to change { bsp.quality }.by(2)
    end
